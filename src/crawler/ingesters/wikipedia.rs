...
pub(crate) async fn process_wikipedia(
    ...
    // pages waiting in the queue for all of them to take something.
    let num_tasks = config.crawler.workers * 2;
    ...
    if last_update < ingester_config.last_update {
        info!(
            "wikipedia dump last updated {}, which is before {} when we last read it.",
            last_update, ingester_config.last_update
        );
        return Ok(());
    }

    // download the big archive to a temp folder somewhere. This must be chunked it will not fit in memory.
    info!("Downloading wikipedia archive - This will take a while.");
...
async fn get_rss_date(client: &mut Client) -> Result<time::OffsetDateTime, Error> {
    ...
    let date_string = channel.items.first().map(|i| i.pub_date()).unwrap_or(None);
...
async fn process_page_inner(config: &Config, page: &Page, index: &Index) -> Result<(), Error> {
    ...
    if page.content.starts_with("#REDIRECT") {
        ...
        Ok(())
    } else if page.title.starts_with("Wikipedia:Articles for deletion") {
        debug!("Skipping {} as its a deletion argument", page.url);
        Ok(())
    } else {
...
fn read_page(
    xml: &mut quick_xml::Reader<BufReader<MultiBzDecoder<File>>>,
        ...
        state = match (state, event) {
            ...
            (state, event) => {
                return Err(Error::InvalidState(format!(
                    "got into state: {state:?} processing event: {event:?}"
                )))
            }
        };
...
mod tests {
    ...
    fn test_filter_between() {
        let cases = [
            ...
            (
                "here {{is a thing {{with nesting stuff}}}} opens",
                "here  opens",
            ),
        ];
    ...
    fn test_square_brackets() {
        let cases = [
            (
                "some text [[an article name|article]] foo bar",
                "some text article foo bar",
            ),
            (
                "some text [[an article name]] foo bar",
                "some text an article name foo bar",
            ),
            (
                "some text [[an article name|something [[nested]]]] foo bar",
                "some text something nested foo bar",
            ),
            (
                "some text [[an article name|something [[with sub bits|nested]]]] foo bar",
                "some text something nested foo bar",
            ),
        ];
