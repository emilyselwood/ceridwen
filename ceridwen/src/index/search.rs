...
pub fn search(root_dir: &str, search_string: &str) -> Result<Vec<SearchResult>, Error> {
    ...
    for word in words.iter() {
        pages.append(&mut word_index::lookup_word(root_dir, word)?);
    }
    ...
    for (url_str, _count) in frequencies.iter() {
        ...
        result.push(page_index::load_page_details(file_path)?);
    }
